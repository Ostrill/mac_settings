...
Настройки и удобные программы

## ⚙️ Настройки

### Активные углы
Наведение мышью на активные углы - очень удобно блокировать экран одним свайпом и расчищать рабочий стол.
> `Настройки` $\to$ `Рабочий стол и Dock` $\to$ `Активные углы` $\to$ Настроить как в таблице:
> ![](images/active_corners.png)

### Строка пути в Finder
По умолчанию Finder не показывает полный путь к открытой папке. Но есть возможность отобразить интерактивный путь.
> `Верхнее меню Finder` $\to$ `Вид` $\to$ `Показать строку пути`
> или через нажатие <kbd>⌥</kbd>+<kbd>⌘</kbd>+<kbd>P</kbd>
...
- `CapsLock` $\to$ `F1`
- `кнопка с микрофоном (F4)` $\to$ `CapsLock` 

А потом в настройках поставил смену языка на <kbd>F1</kbd>. Капс работает как и обычно, но по нажатию на кнопку с микрофоном.
> 1. Для переназначения кнопок нужен .plist файл. Его можно создать [генератором](https://hidutil-generator.netlify.app/), там интерфейс очень простой. Но у нем нет специальных клавиш, таких как кнопка с микрофоном, поэтому пока нужно вместо нее поставить любую другую клавишу.
